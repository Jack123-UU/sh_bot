...
export const sendToReviewTool = createTool({
  ...
  execute: async ({ context, mastra }) => {
    ...
    console.log("🔧 [SendToReviewTool] 开始执行，参数:", JSON.stringify(context, null, 2));
    ...
    try {
      ...
      if (context.hasPhoto && context.photoFileId) {
        ...
        try {
          ...
          console.log('📸 [SendToReviewTool] Telegram API 响应:', JSON.stringify(data, null, 2));
          
          if (!data.ok) {
            throw new Error(`发送图片失败: ${data.description}`);
          }
          reviewMessageId = data.result.message_id;
          console.log('✅ [SendToReviewTool] 图片发送成功, reviewMessageId:', reviewMessageId);
        } catch (error) {
        ...
        try {
          ...
          reviewMessageId = data.result.message_id;
          console.log('✅ [SendToReviewTool] 视频发送成功, reviewMessageId:', reviewMessageId);
        } catch (error) {
        ...
        try {
          ...
          reviewMessageId = data.result.message_id;
          console.log('✅ [SendToReviewTool] 文本发送成功, reviewMessageId:', reviewMessageId);
        } catch (error) {
      ...
      logger?.info('✅ [SendToReviewTool] 已发送到审核频道:', { reviewMessageId });
