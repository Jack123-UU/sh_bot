...
export const forwardMessageTool = createTool({
  ...
  description:
    "Forwards a message to the target Telegram channel with retry mechanism and random delay",
  ...
  inputSchema: z.object({
    ...
    userName: z
      .string()
      .optional()
      .describe("The username of the original sender"),
  }),
  ...
  execute: async ({ context, mastra }) => {
    ...
    logger?.info(
      "🔧 [ForwardMessageTool] Starting execution with params:",
      context,
    );
    ...
    try {
      ...
      if (dbConfig && dbConfig.value) {
        ...
        logger?.info(
          "📝 [ForwardMessageTool] Using target channel from database:",
          targetChannelId,
        );
      } else {
        logger?.info(
          "📝 [ForwardMessageTool] Using target channel from environment variable:",
          targetChannelId,
        );
      }
    } catch (error) {
      logger?.warn(
        "⚠️ [ForwardMessageTool] Failed to read from database, using env variable",
      );
    }
    ...
    for (let attempt = 1; attempt <= MAX_RETRY; attempt++) {
      try {
        logger?.info(
          `📝 [ForwardMessageTool] Attempt ${attempt}/${MAX_RETRY} - Sending message to channel:`,
          {
        ...
        logger?.info(
          "✅ [ForwardMessageTool] Message forwarded successfully:",
          {
        ...
        logger?.error(
          `❌ [ForwardMessageTool] Attempt ${attempt}/${MAX_RETRY} failed:`,
          error,
        );

        // Track consecutive failures
        channelFailures[targetChannelId] =
          (channelFailures[targetChannelId] || 0) + 1;
        ...
        if (attempt === MAX_RETRY) {
          if (channelFailures[targetChannelId] >= FAILURE_ALERT_THRESHOLD) {
            logger?.error(
              `🚨 [ForwardMessageTool] Channel ${targetChannelId} has failed ${channelFailures[targetChannelId]} consecutive times`,
            );
