...
export const replyMessageTool = createTool({
  ...
  description:
    "Sends a reply message to the original Telegram chat, optionally with a reply keyboard (persistent button menu)",
  ...
  inputSchema: z.object({
    ...
    useReplyKeyboard: z
      ...
      .default(false)
      .describe(
        "Whether to show reply keyboard buttons. Set true for /start command.",
      ),
    isAdmin: z
      .boolean()
      .default(false)
      .describe(
        "Whether the user is an admin (shows admin keyboard if true, user keyboard if false)",
      ),
  }),
