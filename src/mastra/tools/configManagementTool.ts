...
export const updateConfigTool = createTool({
  ...
  inputSchema: z.object({
    key: z.enum(['welcome_message', 'drainage_button_text', 'drainage_button_url']).describe("配置项名称"),
  ...
  execute: async ({ context, mastra }) => {
    ...
    try {
      ...
      if (context.requesterId !== adminId) {
        logger?.error('🚫 [UpdateConfigTool] Unauthorized access attempt', { requesterId: context.requesterId });
        return {
          success: false,
...
export const getConfigTool = createTool({
  ...
  inputSchema: z.object({
    requesterId: z.string().optional().describe("请求者的 Telegram user ID（可选，用于日志记录）"),
  }),
  ...
  execute: async ({ context, mastra }) => {
    const logger = mastra?.getLogger();
    logger?.info('🔧 [GetConfigTool] 获取配置', { requesterId: context.requesterId });
    ...
    try {
      ...
      logger?.info('✅ [GetConfigTool] 配置获取成功', { keysCount: Object.keys(config).length });
