...
export const addSourceChannelTool = createTool({
  ...
  description:
    "添加一个监听频道到列表。频道ID格式：纯数字（如 1760505863）会自动转换为 -1001760505863，或使用 @username 格式",

  inputSchema: z.object({
    channelId: z
      .string()
      .describe("频道ID（纯数字如 1760505863，或 @username）"),
...
export const listSourceChannelsTool = createTool({
  ...
  execute: async ({ mastra }) => {
    ...
    try {
      ...
      logger?.info("✅ [ListSourceChannelsTool] 获取成功，共", {
