...
export const reviewCallbackTool = createTool({
  ...
  inputSchema: z.object({
    ...
    callbackData: z.string().describe("回调数据，格式：approve/reject/settings"),
  ...
  execute: async ({ context, mastra }) => {
    ...
    if (!isPrimaryAdmin) {
      ...
      if (cachedStatus !== null) {
        ...
        logger?.info('⚡ [ReviewCallbackTool] 管理员状态从缓存获取:', { userId: context.userId, isDbAdmin });
      } else {
        // 缓存未命中，查询数据库
        logger?.info('🔍 [ReviewCallbackTool] 缓存未命中，查询数据库...');
        const adminCheck = await db.select().from(admins).where(eq(admins.userId, context.userId)).limit(1);
        isDbAdmin = adminCheck.length > 0;
        
        // 更新缓存
        adminCache.set(context.userId, isDbAdmin);
        logger?.info('💾 [ReviewCallbackTool] 管理员状态已缓存:', { userId: context.userId, isDbAdmin });
      }
    }
    ...
    if (!isAdmin) {
      logger?.warn('⚠️ [ReviewCallbackTool] 非管理员尝试操作:', { userId: context.userId, isPrimaryAdmin, isDbAdmin });
    ...
    logger?.info('✅ [ReviewCallbackTool] 管理员权限验证通过:', { userId: context.userId, isPrimaryAdmin, isDbAdmin });
    ...
    try {
      if (action === "approve") {
        ...
        // 1. 转发消息到目标频道
        const forwardResponse = await fetch(`https://api.telegram.org/bot${botToken}/forwardMessage`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
        ...
        // 2. 获取欢迎语和引流按钮
        const configResult = await db.select().from(botConfig).where(eq(botConfig.key, 'welcome_message'));
        const welcomeMessage = configResult[0]?.value || '欢迎使用我们的服务！👋';
        
        const buttonsResult = await db.select().from(referralButtons).orderBy(referralButtons.displayOrder, referralButtons.id);
        
        logger?.info('📋 [ReviewCallbackTool] 获取到引流按钮:', { count: buttonsResult.length });
        ...
        if (inlineKeyboard.length > 0) {
          const welcomeResponse = await fetch(`https://api.telegram.org/bot${botToken}/sendMessage`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              chat_id: targetChannelId,
          ...
          logger?.info('💬 [ReviewCallbackTool] 欢迎消息发送结果:', welcomeData);
        }
