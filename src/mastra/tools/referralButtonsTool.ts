...
export const addReferralButtonTool = createTool({
  ...
  inputSchema: z.object({
    ...
    buttonUrl: z.string().describe("按钮跳转链接（例如：https://t.me/hongqi168888）"),
    displayOrder: z.number().optional().describe("显示顺序，数字越小越靠前，默认为0"),
    requesterId: z.string().describe("请求者的 Telegram user ID（必须是管理员）"),
  }),
  ...
  execute: async ({ context, mastra }) => {
    ...
    try {
      ...
      if (!requesterCheck) {
        logger?.error('🚫 [AddReferralButtonTool] Unauthorized access attempt', { requesterId: context.requesterId });
        return {
          success: false,
      ...
      logger?.info('✅ [AddReferralButtonTool] 引流按钮添加成功', { buttonId: result.id });
...
export const removeReferralButtonTool = createTool({
  ...
  execute: async ({ context, mastra }) => {
    ...
    try {
      ...
      if (!requesterCheck) {
        logger?.error('🚫 [RemoveReferralButtonTool] Unauthorized access attempt', { requesterId: context.requesterId });
        return {
          success: false,
...
export const listReferralButtonsTool = createTool({
  ...
  inputSchema: z.object({
    requesterId: z.string().optional().describe("请求者的 Telegram user ID（可选，用于日志）"),
  }),
  ...
  execute: async ({ context, mastra }) => {
    const logger = mastra?.getLogger();
    logger?.info('🔧 [ListReferralButtonsTool] 列出引流按钮', { requesterId: context.requesterId });
    ...
    try {
      ...
      logger?.info('✅ [ListReferralButtonsTool] 成功获取引流按钮列表', { count: buttons.length });
...
export const updateReferralButtonOrderTool = createTool({
  ...
  execute: async ({ context, mastra }) => {
    ...
    try {
      ...
      if (!requesterCheck) {
        logger?.error('🚫 [UpdateReferralButtonOrderTool] Unauthorized access attempt', { requesterId: context.requesterId });
        return {
          success: false,
