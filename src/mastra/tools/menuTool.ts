...
export const sendMainMenuTool = createTool({
  id: "send-main-menu",
  description:
    "Sends the main menu or welcome message to the user. Admins see management menu, regular users see welcome message with referral buttons.",
  ...
  execute: async ({ context, mastra }) => {
    ...
    logger?.info("🔧 [MenuTool] Sending menu/welcome", {
    ...
    try {
      ...
      if (context.isAdmin) {
        ...
        keyboard = {
          inline_keyboard: [
            [
              ...
              {
                text: "🔘 引流按钮管理",
                callback_data: "menu_referral_buttons",
              },
            ],
            [
        ...
        logger?.info("📝 [MenuTool] Found referral buttons", {
          count: referralButtons.length,
        });
...
export const sendKeywordMenuTool = createTool({
  ...
  execute: async ({ context, mastra }) => {
    const logger = mastra?.getLogger();
    logger?.info("🔧 [KeywordMenuTool] Sending keyword menu", {
      chatId: context.chatId,
    });
      ...
      const keyboard = {
        inline_keyboard: [
          ...
          [
            {
              text: "📋 查看所有关键词",
              callback_data: "action_list_keywords",
            },
          ],
          [
            {
              text: "⚙️ 设置匹配数量",
              callback_data: "action_set_min_keywords",
            },
          ],
...
export const sendChannelMenuTool = createTool({
  ...
  execute: async ({ context, mastra }) => {
    const logger = mastra?.getLogger();
    logger?.info("🔧 [ChannelMenuTool] Sending channel menu", {
      chatId: context.chatId,
    });
...
export const sendReferralButtonsMenuTool = createTool({
  ...
  description:
    "Sends the referral buttons management menu with interactive buttons",
  ...
  execute: async ({ context, mastra }) => {
    const logger = mastra?.getLogger();
    logger?.info("🔧 [ReferralButtonsMenuTool] Sending referral buttons menu", {
      chatId: context.chatId,
    });
      ...
      const keyboard = {
        inline_keyboard: [
          [
            {
              text: "➕ 添加按钮",
              callback_data: "action_add_referral_button",
            },
          ],
          [
            {
              text: "➖ 删除按钮",
              callback_data: "action_remove_referral_button",
            },
          ],
          [
            {
              text: "📋 查看所有按钮",
              callback_data: "action_list_referral_buttons",
            },
          ],
          [
            {
              text: "🔄 调整顺序",
              callback_data: "action_reorder_referral_buttons",
            },
          ],
          [{ text: "🔙 返回主菜单", callback_data: "menu_main" }],
        ],
...
export const sendAdminMenuTool = createTool({
  ...
  execute: async ({ context, mastra }) => {
    const logger = mastra?.getLogger();
    logger?.info("🔧 [AdminMenuTool] Sending admin menu", {
      chatId: context.chatId,
    });
...
export const answerCallbackQueryTool = createTool({
  ...
  inputSchema: z.object({
    ...
    showAlert: z
      .boolean()
      .optional()
      .describe("Whether to show an alert instead of a notification"),
  }),
