...
export const checkIsAdminTool = createTool({
  id: "check-is-admin",
  description: "Checks if a user ID is an administrator (checks both environment variable ADMIN_ID and database)",
  ...
  execute: async ({ context, mastra }) => {
    ...
    logger?.info('🔧 [CheckIsAdminTool] Checking admin status', { userId: context.userId });
    
    try {
      const envAdminId = process.env.ADMIN_ID;
      
      if (context.userId === envAdminId) {
        logger?.info('✅ [CheckIsAdminTool] User is primary admin (ADMIN_ID)', { userId: context.userId });
        return { isAdmin: true };
      }
      ...
      if (cachedStatus !== null) {
        logger?.info('⚡ [CheckIsAdminTool] Admin status from cache:', { userId: context.userId, isAdmin: cachedStatus });
      ...
      logger?.error('❌ [CheckIsAdminTool] Error checking admin status:', error);
...
export const addAdminTool = createTool({
  id: "add-admin",
  description: "Adds a new administrator and sends confirmation message. REQUIRES ADMIN PERMISSION.",
  ...
  inputSchema: z.object({
    ...
    requesterId: z.string().describe("The Telegram user ID of the requester (must be admin)"),
    chatId: z.string().describe("The chat ID to send the confirmation message to"),
  }),
  ...
  execute: async ({ context, mastra }) => {
    const logger = mastra?.getLogger();
    logger?.info('🔧 [AddAdminTool] Adding admin', { userId: context.userId, requesterId: context.requesterId });
    ...
    try {
      ...
      if (!isRequesterPrimaryAdmin && !requesterCheck) {
        logger?.error('🚫 [AddAdminTool] Unauthorized access attempt', { requesterId: context.requesterId });
      ...
      await db.none(
        ...
        [context.userId, context.username || null]
      );
      
      // 🗑️ 清除缓存，确保下次查询时获取最新状态
      adminCache.invalidate(context.userId);
      logger?.info('🗑️ [AddAdminTool] Cache invalidated for user:', { userId: context.userId });
...
export const removeAdminTool = createTool({
  id: "remove-admin",
  description: "Removes an administrator and sends confirmation message. REQUIRES ADMIN PERMISSION.",
  ...
  execute: async ({ context, mastra }) => {
    const logger = mastra?.getLogger();
    logger?.info('🔧 [RemoveAdminTool] Removing admin', { userId: context.userId, requesterId: context.requesterId });
    ...
    try {
      ...
      if (!isRequesterPrimaryAdmin && !requesterCheck) {
        logger?.error('🚫 [RemoveAdminTool] Unauthorized access attempt', { requesterId: context.requesterId });
      ...
      // 🗑️ 清除缓存，确保下次查询时获取最新状态
      adminCache.invalidate(context.userId);
      logger?.info('🗑️ [RemoveAdminTool] Cache invalidated for user:', { userId: context.userId });
...
export const listAdminsTool = createTool({
  id: "list-admins",
  description: "Lists all administrators and sends the formatted list to user. REQUIRES ADMIN PERMISSION.",
  ...
  execute: async ({ context, mastra }) => {
    const logger = mastra?.getLogger();
    logger?.info('🔧 [ListAdminsTool] Listing all admins', { requesterId: context.requesterId });
    ...
    try {
      ...
      if (!isRequesterPrimaryAdmin && !requesterCheck) {
        logger?.error('🚫 [ListAdminsTool] Unauthorized access attempt', { requesterId: context.requesterId });
      ...
      logger?.info('✅ [ListAdminsTool] Admin list sent successfully', { count: results.length + 1 });
