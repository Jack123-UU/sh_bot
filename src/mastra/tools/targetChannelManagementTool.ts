...
export const targetChannelManagementTool = createTool({
  ...
  description:
    "管理目标频道（最终消息接收频道）：查看当前目标频道或设置新的目标频道",
    ...
    try {
      if (context.action === "view") {
        ...
        if (result.length === 0) {
          ...
          logger?.info("📋 [TargetChannelManagementTool] 使用环境变量:", {
            envChannelId,
          });
          return {
            success: true,
        ...
        logger?.info("✅ [TargetChannelManagementTool] 查看成功:", {
          channelId,
        });
        return {
          success: true,
        ...
        if (existing.length > 0) {
          ...
          logger?.info("✅ [TargetChannelManagementTool] 目标频道已更新:", {
            channelId,
          });
          return {
            success: true,
          ...
          logger?.info("✅ [TargetChannelManagementTool] 目标频道已设置:", {
            channelId,
          });
          return {
            success: true,
