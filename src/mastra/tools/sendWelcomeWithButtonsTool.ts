...
export const sendWelcomeWithButtonsTool = createTool({
  ...
  description:
    "Sends a welcome message with referral buttons and optional reply keyboard to a Telegram chat",
  ...
  inputSchema: z.object({
    ...
    useReplyKeyboard: z
      .boolean()
      .default(false)
      .describe("Whether to show reply keyboard buttons"),
    isAdmin: z
      .boolean()
      .default(false)
      .describe("Whether the user is an admin (determines keyboard layout)"),
  }),
  ...
  execute: async ({ context, mastra }) => {
    ...
    logger?.info(
      "🔧 [SendWelcomeWithButtonsTool] Starting execution with params:",
      context,
    );
    ...
    if (!botToken) {
      logger?.error(
        "❌ [SendWelcomeWithButtonsTool] Missing TELEGRAM_BOT_TOKEN",
      );
      return {
        success: false,
    ...
    try {
      ...
      logger?.info(
        "📝 [SendWelcomeWithButtonsTool] Sending message with buttons:",
        {
      ...
      if (!response.ok || !data.ok) {
        logger?.error(
          "❌ [SendWelcomeWithButtonsTool] Telegram API error:",
          data,
        );
        return {
          success: false,
      ...
      logger?.info(
        "✅ [SendWelcomeWithButtonsTool] Message sent successfully:",
        {
      ...
      logger?.error(
        "❌ [SendWelcomeWithButtonsTool] Exception occurred:",
        error,
      );
      return {
        success: false,
